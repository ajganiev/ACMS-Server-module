...
void vecAdd(const double *vec1, const double *vec2, double *res, int dim) {
    int i = 0;
    for (; i < dim; i++) {
        res[i] = vec1[i] + vec2[i];
    }
...
void vecMultiplyNum(const double *vec1, const double num, double *res, int dim) {
    int i = 0;
    for (; i < dim; i++) {
        res[i] = vec1[i] * num;
    }
...
void vecAddNum(const double *vec1, const double num, double *res, int dim) {
    int i = 0;
    for (; i < dim; i++) {
        res[i] = vec1[i] + num;
    }
...
void vecSubtractNum(const double *vec1, const double num, double *res, int dim) {
    int i = 0;
    for (; i < dim; i++) {
        res[i] = vec1[i] - num;
    }
...
void vecSubtract(const double *vec1, const double *vec2, double *res, int dim) {
    int i = 0;
    for (; i < dim; i++) {
        res[i] = vec1[i] - vec2[i];
    }
...
void movePlanes(p_planes *data, double deltaTime) {
    int i = 0;
    for (; i < data->size; i++) {
        double temp[3];
        vecMultiplyNum(data->planes[i].velocity, deltaTime*0.1, temp, 2);
        vecAdd(data->planes[i].position, temp, data->planes[i].position, 2);
    }
...
void worker(void *args) {
    ...
    while (1) {
        g_msg msg;
        p_planes *data = &testData;

        movePlanes(data, deltaTime);

        prepare_packet(36, "server-001", &msg, data, sizeof(*data));
